<p align="center">
  <a href="https://github.com/FlorentinaSL/PurgaLibFramework">Repo Link</a>
</p>

<p align="center">
  <img width="470" height="383" alt="ico" src="https://github.com/user-attachments/assets/7d7d34ad-4734-4ead-b236-58c4a3c9b614" />
</p>

<h1 align="center">
🔥 PurgaLib Framework
</h1>

<p align="center">
  <img src="https://img.shields.io/badge/Status-Release-green?style=for-the-badge&logo=testinglibrary"/>
  <img src="https://img.shields.io/badge/Platform-SCP:_Secret_Laboratory-black?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Language-C%23-239120?style=for-the-badge&logo=csharp&logoColor=white"/>
  <img src="https://img.shields.io/badge/Build-Passing-brightgreen?style=for-the-badge&logo=githubactions"/>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Framework-Custom-blueviolet?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/API-Stable_Core-success?style=for-the-badge"/>
</p>

---

<p align="center">
  PurgaLib is an advanced library/framework for SCP:SL that adds extended server features, custom role management, extended events, and automatic systems like the built-in updater.
</p>

---

<h2 align="center">📖 Description</h2>

<p>
PurgaLib provides:
<ul>
<li>Custom role system and permission management</li>
<li>Extended events for players, rounds, doors, elevators, Tesla interactions, etc.</li>
<li>API for plugin integration</li>
<li>Automatic updater via GitHub Releases</li>
<li>Cross-platform server support (Windows/Linux)</li>
<li>Verified player recognition and automatic credits system</li>
</ul>
</p>
